...
OUTDIR="/scratch/users/snarayanasamy/IMP_MS_data/mappingSingleOmics/MOCAT"
...
do
  ...
  ls $MG2MGMT
  samtools flagstat $MG2MGMT > ${OUTDIR}/$S/MG_reads-x-_MGMT-assm.flagstat.txt

  ls $MT2MGMT
  samtools flagstat $MG2MGMT > ${OUTDIR}/$S/MT_reads-x-_MGMT-assm.flagstat.txt

  ls $MG2MG
  samtools flagstat $MG2MG > ${OUTDIR}/$S/MG_reads-x-_MG-assm.flagstat.txt

  ls $MT2MG
  samtools flagstat $MT2MG > ${OUTDIR}/$S/MT_reads-x-_MG-assm.flagstat.txt

  ls $MG2MT
  samtools flagstat $MG2MT > ${OUTDIR}/$S/MG_reads-x-_MT-assm.flagstat.txt

  ls $MT2MT
  samtools flagstat $MT2MT > ${OUTDIR}/$S/MT_reads-x-_MT-assm.flagstat.txt
