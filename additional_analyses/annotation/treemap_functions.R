    ...
    FALSE), main = "Tree Map", print = TRUE, pal="Blues", textcol="white", linecol="grey") 
{
    ...
    bpal <- c(brewer.pal(7, pal))
    color.ramp.pos <- colorRamp(bpal, space="Lab")
    color.ramp.neg <- colorRamp(bpal, space="Lab")
        ...
        for (s in 1:length(stock.viewports)) {
            ...
            if (lab[2]) {
                stock.tree <- addGrob(stock.tree, textGrob(x = unit(1, 
                  ...
                  label = this.data$label[s], gp = gpar(col = textcol), 
                  name = "label", just = c("left", "top")))
            }
            group.tree <- addGrob(group.tree, stock.tree)
        }
        group.tree <- addGrob(group.tree, rectGrob(gp = gpar(col = linecol), 
            name = "border"))
        if (lab[1]) {
            group.tree <- addGrob(group.tree, textGrob(label = group.data$label[i], 
                name = "label", gp = gpar(col = textcol)))
        }
