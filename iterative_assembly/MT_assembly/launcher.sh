...
do
    ...
    OUTDIR="/scratch/users/snarayanasamy/IMP_MS_data/iterative_assemblies/MT_assemblies/${S}"
    OUTLOG="/scratch/users/snarayanasamy/IMP_MS_data/iterative_assemblies/MT_assemblies/${S}/${S}_MT-iterative.log"
    TMPDIR="/scratch/users/snarayanasamy/IMP_MS_data/iterative_assemblies/MT_assemblies/${S}/tmp"
    
    #CMD=${OARSUB} -n "${S}_MT_iterative" "./execution.sh $INDIR $OUTDIR $OUTLOG $TMPDIR"
    CMD="${OARSUB} -n \"${S}_MT_iterative\" \"./execution.sh $INDIR $OUTDIR $OUTLOG $TMPDIR\""
