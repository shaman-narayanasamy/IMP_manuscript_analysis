...
do
    ...
    paste <(echo ${S}) <(echo IMP) <(tail -n1 $IMP_MGMT/gene_count.Prodigal.tsv)
    #paste <(echo ${S}) <(echo IMP-megahit) <(tail -n1 $IMP_MGMT/gene_count.Prodigal.tsv)
    paste <(echo ${S}) <(echo MOCAT_MGMT) <(tail -n1 $MOCAT_MGMT/gene_count.Prodigal.tsv)
    #paste <(echo ${S}) <(echo MetAMOS_MGMT) <(tail -n1 $METAMOS_MGMT/gene_count.Prodigal.tsv)

    paste <(echo ${S}) <(echo IMP_MG) <(tail -n1 $IMP_MG/gene_count.Prodigal.tsv)
    paste <(echo ${S}) <(echo MOCAT_MG) <(tail -n1 $MOCAT_MG/gene_count.Prodigal.tsv)
    #paste <(echo ${S}) <(echo MetAMOS_MG) <(tail -n1 $METAMOS_MG/gene_count.Prodigal.tsv)

    paste <(echo ${S}) <(echo IMP_MT) <(tail -n1 $IMP_MT/gene_count.Prodigal.tsv)
    paste <(echo ${S}) <(echo MOCAT_MT) <(tail -n1 $MOCAT_MT/gene_count.Prodigal.tsv)
    #paste <(echo ${S}) <(echo MetAMOS_MT) <(tail -n1 $METAMOS_MT/gene_count.Prodigal.tsv)
