...
do
  ...
  then 
    ...
    ${OARSUB} -n "${S}_metAmos_MGMT_prodigal" "./makeGenePredictionsProdigal.sh $ $MGMT_REF $OUTDIR"

    OUTDIR_MG="/scratch/users/snarayanasamy/IMP_MS_data/prodigal_analysis/metAmos_MG/${S}"
    ${OARSUB} -n "${S}_metAmos_MG_prodigal" "./makeGenePredictionsProdigal.sh $ $MG_REF $OUTDIR"

    OUTDIR_MT="/scratch/users/snarayanasamy/IMP_MS_data/prodigal_analysis/metAmos_MT/${S}"
    ${OARSUB} -n "${S}_metAmos_MT_prodigal" "./makeGenePredictionsProdigal.sh $ $MT_REF $OUTDIR"
